...
.master-header-top{{display:flex;align-items:center;justify-content:space-between;padding:14px 40px}}
...
.tab-btn{{padding:10px 28px;font-size:13px;font-weight:600;color:rgba(255,255,255,.6);cursor:pointer;border:none;background:none;font-family:var(--font);letter-spacing:-.2px;border-bottom:3px solid transparent;transition:all .2s}}
...
.chart-container,.chart-panel{{background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius);padding:24px;margin-bottom:32px;box-shadow:var(--shadow);position:relative}}
...
/* FIX: Remove max-height constraint — let charts size based on data count */
canvas{{display:block;width:100%!important}}

/* TABLE STYLES */
table{{width:100%;border-collapse:collapse;font-size:12px}}
th{{background:var(--navy);color:#fff;padding:10px 12px;text-align:left;font-weight:600;font-size:11px;text-transform:uppercase;letter-spacing:.5px;position:sticky;top:0;z-index:10}}
...
<script>
// ============================================================
// DOWNLOAD UTILITY — export any canvas as PNG
// ============================================================
...
// Add download buttons to all chart containers after charts init
function addDownloadButtons() {{
  document.querySelectorAll('canvas').forEach(canvas => {{
    const parent = canvas.closest('.chart-container, .chart-panel');
    if (!parent || parent.querySelector('.dl-btn')) return;
    const btn = document.createElement('button');
    btn.className = 'dl-btn';
    btn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg> PNG';
    const name = (parent.querySelector('.chart-title, h3')?.textContent || canvas.id).replace(/[^a-zA-Z0-9]/g,'_').substring(0,40);
    btn.onclick = () => downloadChart(canvas.id, 'NLB_' + name);
...
// ============================================================
// TABLE DOWNLOAD — export any visible table as CSV
// ============================================================
...
// Add CSV download to table wrappers
function addTableDownloadButtons() {{
  document.querySelectorAll('.tbl-wrap, .table-container').forEach((wrap, i) => {{
    if (wrap.querySelector('.dl-btn')) return;
    ...
    btn.innerHTML = '<svg viewBox="0 0 24 24"><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg> CSV';
    const name = 'NLB_Table_' + (i+1);
    btn.onclick = () => downloadTableCSV(tbl, name);
...
function switchModule(n) {{
  ...
  if (!chartsInitialized[n]) {{
    ...
    // Add download buttons after charts render
    setTimeout(() => {{
      addDownloadButtons();
...
// ============================================================
// FIX CHART HEIGHTS — set proper container heights for horizontal bars
// ============================================================
function fixChartHeights() {{
  // For horizontal bar charts, height should be ~28px per bar + 80px padding
  document.querySelectorAll('canvas').forEach(canvas => {{
    const chart = Chart.getChart(canvas);
    if (!chart) return;
    const isHorizontal = chart.config?.options?.indexAxis === 'y';
    if (isHorizontal) {{
      const count = chart.data?.labels?.length || 10;
      const h = Math.max(400, count * 28 + 80);
...
{js1}
setTimeout(fixChartHeights, 300);
}}
